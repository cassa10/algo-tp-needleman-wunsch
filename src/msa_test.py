...
def parse_fasta_and_validate(file_dir, score_mtx):
    ...
    msa_score, msa_alignments = msa.init([seq_A, seq_B], file_dir, gap_penalty)
    ...
    print(f" nw(seq_A) == msa(seq_A) => {nw_alignment_seq_A == msa_alignment_seq_A}")
    print(f" nw(seq_B) == msa(seq_B) => {nw_alignment_seq_B == msa_alignment_seq_B}")

    return nw_score == msa_score
